  ...
  return (
    <div className="flex flex-col md:flex-row gap-4 items-center">
      ...
      <div className="relative flex-1 w-full group">
        <Search className={`absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 transition-colors ${isPending ? 'text-[#80B828] animate-pulse' : 'text-gray-500 group-focus-within:text-[#80B828]'}`} />
        <input
          type="text"
          placeholder="Buscar departamento..."
          value={query}
          onChange={(e) => setQuery(e.target.value)}
          className="w-full bg-[#111111] border border-[#ffffff0a] focus:border-[#80B828]/50 rounded-xl pl-11 pr-4 py-2.5 text-sm text-white outline-none transition-all placeholder-gray-600 focus:ring-1 focus:ring-[#80B828]/20"
        />
        {query && (
          <button 
            onClick={() => setQuery('')}
